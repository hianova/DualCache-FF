...
impl<K, V, const N: usize> Arena<K, V, N> {
    pub fn new() -> Self {
        let mut next_free = core::array::from_fn(|_| core::sync::atomic::AtomicU32::new(0));
        let mut i = 0;
        while i < N - 1 {
    ...
    pub fn alloc(&self, key: K, value: V, node: *mut crate::core::qsbr::ThreadStateNode) -> Option<usize> {
        ...
        if let Some(idx) = local_free.pop() {
            unsafe {
                (*self.nodes.get_unchecked(idx as usize).get()).write(Node { key, value });
            }
        ...
        loop {
            ...
            let tag = head >> 32;
            let next = unsafe { self.next_free.get_unchecked(index as usize) }.load(Ordering::Relaxed) as u32;
    ...
    pub unsafe fn free(&self, index: usize) {
        ...
        loop {
            let old_index = (head & 0xFFFFFFFF) as u32;
            unsafe { self.next_free.get_unchecked(index) }.store(old_index, Ordering::Relaxed);
            let tag = head >> 32;
            let new_head = (tag.wrapping_add(1) << 32) | index;
